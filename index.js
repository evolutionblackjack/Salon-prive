...
function sndSit(){if(!pref.snd)return;noise(.16,140,'lowpass',.34);setTimeout(()=>noise(.1,300,'bandpass',.2),50);setTimeout(()=>beep(130,.1,.05),90);setTimeout(()=>noise(.06,500,'lowpass',.12),140);}
let ambN=null,ambG=null;
function stopAmb(){try{if(ambN)ambN.stop();}catch(e){}ambN=null;ambG=null;}
function setAmb(v){pref.amb=v;savePref();if(typeof paintPref==='function')paintPref();stopAmb();if(v==='off')return;try{ensureAudio();const len=actx.sampleRate*6;const n=actx.createBuffer(2,len,actx.sampleRate);for(let ch=0;ch<2;ch++){const d=n.getChannelData(ch);let last=0;for(let i=0;i<d.length;i++){last=last*.995+(Math.random()*2-1)*.03;const talk=Math.sin(i*0.0013+ch*1.7)*0.06+Math.sin(i*0.0004+ch)*0.05;d[i]=last*0.65+talk;}}const s=actx.createBufferSource();s.buffer=n;s.loop=true;const f=actx.createBiquadFilter();f.type='lowpass';f.frequency.value=1100;const g=actx.createGain();g.gain.value=.14;s.connect(f);f.connect(g);g.connect(actx.destination);s.start();ambN=s;ambG=g;}catch(e){}}
...
function ui(s,skipDeal){
  ...
  if((s.phase==='bet'||s.phase==='end')&&seated){
    [5,10,25,50].forEach(v=>{const col=v===5?'#2a2a2a':v===10?'#1e4ec4':v===25?'#1a8a3a':'#c42838';const b=document.createElement('button');b.className='chip c'+v;b.innerHTML=svgChip(v,col);b.onclick=()=>{if(lastBet+v>50){alert('Mise max 50');return;}lastBet+=v;sndChip();const mid=$('chipon');mid.innerHTML=svgChip(lastBet,lastBet>=50?'#c42838':lastBet>=25?'#1a8a3a':lastBet>=10?'#1e4ec4':'#2a2a2a');mid.className='chipon';};chips.appendChild(b);});
