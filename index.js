...
async function api(req,res,path){
  ...
  if(path==='/api/admin'&&u.role==='admin'){
    ...
    if(b.supprimer){const c=comptes.get(b.supprimer);if(c&&c.role!=='admin'){const i=mine(c.id);if(i>=0)kick(i,true);for(const [tk,uid] of [...sessions])if(uid===c.id)sessions.delete(tk);comptes.delete(c.id);}}
...
<h1>BLACKJACK</h1>
<input id="ps" placeholder="Pseudo" autocomplete="username"><input id="cd" type="password" placeholder="Code" autocomplete="current-password">
<label style="display:flex;gap:.4rem;align-items:center;justify-content:center;margin:.4rem 0;font-size:.78rem;color:#c8c0b0"><input id="mem" type="checkbox" checked style="width:auto"> Enregistrer le code</label>
...
const list=$('clist');list.innerHTML='';d.comptes.forEach(c=>{const r=document.createElement('div');r.className='row';r.innerHTML='<span>'+c.pseudo+' / '+c.code+' · €'+c.solde+'</span>';const i=document.createElement('input');i.type='number';i.placeholder='+/-';const ok=document.createElement('button');ok.className='g';ok.textContent='OK';ok.onclick=async()=>{await api('/api/admin',{jetons:{id:c.id,delta:+i.value||0}});$('admBtn').click();};r.appendChild(i);r.appendChild(ok);
    if(c.role!=='admin'){const del=document.createElement('button');del.textContent='Suppr';del.style.background='#5a1212';del.style.color='#fff';del.onclick=async()=>{if(!confirm('Supprimer '+c.pseudo+' ?'))return;await api('/api/admin',{supprimer:c.id});$('admBtn').click();};r.appendChild(del);}
    list.appendChild(r);});};
...
if(localStorage.getItem('bj.ps')){$('ps').value=localStorage.getItem('bj.ps');$('cd').value=localStorage.getItem('bj.cd')||'';}
